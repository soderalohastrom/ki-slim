...
$SalesPerson_user_id = $_SESSION['system_user_id'];
...
WHERE
	1
	AND
		PersonsSalesCommissions.Users_user_id='".$SalesPerson_user_id."'
AND
...
if(!isset($scu_snd['empty_result'])):
		$sales	=	getSalesCount($SalesPerson_user_id, $enderEpoch, $startEpoch);		
		if($appt == 0) {
			$dolper = 0;
		} else {
		...
		$newsql = "SELECT *, CONCAT(FirstName,' ',LastName) as Name FROM PersonsSales INNER JOIN Persons ON Persons.Person_id=PersonsSales.Persons_Person_id INNER JOIN PersonsProfile ON PersonsProfile.Person_id=Persons.Person_id INNER JOIN PersonsSalesCommissions ON PersonsSalesCommissions.PersonsSales_PersonsSales_id=PersonsSales.PersonsSales_id WHERE (PersonsSales_dateCreated >= '".$startEpoch."' AND PersonsSales_dateCreated <= '".$enderEpoch."') AND PersonsSalesCommissions.Users_user_id='".$SalesPerson_user_id."'";
		?>
		<tr>
			<td><a href="javascript:;" onclick="previewRecords('Sales for <?php echo $RECORD->get_FulluserName($SalesPerson_user_id)?> between <?php echo $_POST['filterDates']?>','<?php echo $ENC->encrypt($newsql)?>')"><?php echo $RECORD->get_FulluserName($SalesPerson_user_id)?></a></td>
