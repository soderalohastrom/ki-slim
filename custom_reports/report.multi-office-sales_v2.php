...
<div class="row">
    ...
    <div class="col-4">
        ...
        <div class="input-group m-input-group">					
            <span class="input-group-addon" id="basic-addon1"><i class="flaticon-tool-1"></i> &nbsp;&nbsp;Version</span>
            <select id="version" name="version" class="form-control m-input input-sm" style="height:38px;">
            	<option value="sales" <?php echo (($_POST['version'] == 'sales')? 'selected':'')?>>Sales</option>
                <option value="agreements" <?php echo (($_POST['version'] == 'agreements')? 'selected':'')?>>Agreements</option>
            </select>
        </div>
...
<thead class="thead-inverse">
	<tr>
    	<th colspan="<?php echo (($_POST['version'] == 'sales')? '7':'8')?>">&nbsp;</th>
...
foreach($offices as $office):
	...
	if($_POST['version'] == 'sales') {
		...
		AND
			PersonsPaymentInfo.PaymentInfo_Status NOT IN (5, 6)
		GROUP BY 
			PaymentInfo_ID 
		ORDER BY 
		...
		FROM
			PersonsPaymentInfo
			LEFT JOIN PersonsContract ON PersonsContract.Contract_id=PersonsPaymentInfo.Contract_id
			LEFT JOIN PersonsSales ON PersonsContract.Contract_id=PersonsSales.PersonsSales_ContractID
	...
	if($_POST['version'] == 'agreements'):
		...
		<tr id="prow_<?php echo $office?>">
			<td colspan="<?php echo (($_POST['version'] == 'sales')? '6':'8')?>" class="office-name-block"><span style="font-size:16px; font-weight:bold;"><?php echo $RECORD->getOfficeName($office)?></span> <i class="toggle-section la la-angle-up" data-id="<?php echo $office?>"></i></td>
		...
		if(isset($s_snd['empty_result'])):
			...
			foreach($s_snd as $s_dta):
				...
				if($s_dta['PaymentInfo_Status'] == 3):
					$processed = $ENC->decrypt($s_dta['PaymentInfo_Amount']);
				else:
					$processed = '0.00';
				endif;
				
				if($s_dta['PaymentInfo_Status'] == 4):
					$paid = $ENC->decrypt($s_dta['PaymentInfo_Amount']);
				else:
					$paid = '0.00';
				endif;
				
				if($s_dta['PaymentInfo_isRefund']):
					$refund = $ENC->decrypt($s_dta['PaymentInfo_Amount']);
				else:
					$refund = '0.00';
				endif;
				
				$balance = $ENC->decrypt($s_dta['PaymentInfo_Amount']) - $paid;
				?>
				<tr id="row_<?php echo $s_dta['PersonsSales_id']?>" class="<?php echo $rowClass?>">
					<td width="75"><?php echo date("m/d/y", $s_dta['PaymentInfo_Execute'])?></td>
					<td width="325">
						<a href="/profile/<?php echo $s_dta['Persons_Person_id']?>" class="m-link" style="color:#000;" target="_blank"><?php echo $RECORD->get_personName($s_dta['Persons_Person_id'])?> <i class="la la-user"></i></a>				
					</td>
					<td width="2">
					...
					<td width="30" class="text-center"><?php echo $s_dta['Persons_Person_id']?></td>
					<td width="50" class="text-center"><?php echo (($s_dta['PersonsSales_ContractID'] == 0)? '<i class="flaticon-warning-2" data-toggle="m-tooltip" title="No Agreement is associated with this sale"></i>':'<i class="fa fa-check m--font-success" data-toggle="m-tooltip" title="Agreement Record Found"></i>')?></td>
					<td width="250"><?php echo @implode(", ", $sp_array)?></td>
					<td width="150" class="text-center" id="contractStatus_<?php echo $s_dta['PersonsSales_ContractID']?>"><?php echo $SALES->getPaymentStatus($s_dta['PaymentInfo_Status'])?></td>           
					
                    <!--<td width="100" class="text-right"><?php echo number_format($ENC->decrypt($s_dta['PaymentInfo_Amount']), 2)?> <?php echo (($s_dta['PaymentInfo_isRefund'])? '[REFUND]':'')?></td>-->
					<!--<td width="10%"><?php echo $s_dta['PersonsSales_taxes']?></td>-->
                                
					<td width="100" class="text-right"><?php echo number_format($ENC->decrypt($s_dta['PaymentInfo_Amount']), 2)?></td>
					<td width="100" class="text-right"><?php echo number_format($processed, 2)?></td>
					<td width="100" class="text-right"><a href="javascript:reviewSalePayments('<?php echo $s_dta['PersonsSales_id']?>','<?php echo $s_dta['Persons_Person_id']?>')" class="m-link" style="color:#000;"><?php echo number_format($paid, 2)?></a></td>
					<td width="100" class="text-right"><?php echo number_format($refund, 2)?></td>
					<td width="100" class="text-right"><?php echo number_format($balance, 2)?></td>  
				</tr> 
				<?php
			$total_base[] = $s_dta['PersonsSales_basePrice'];
			$total_tax[] = $s_dta['PersonsSales_taxes'];
			$total_dollars[] = $ENC->decrypt($s_dta['PaymentInfo_Amount']);
			$total_paid[] = $paid;
			$total_balance[] = $balance;
			$total_refund[] = $refund;
			$total_process[] = $processed;
		...
		?>
		<tr id="prow_<?php echo $office?>">
			<td colspan="<?php echo (($_POST['version'] == 'sales')? '7':'8')?>" class="office-name-block"><span style="font-size:16px; font-weight:bold;"><?php echo $RECORD->getOfficeName($office)?></span> <i class="toggle-section la la-angle-up" data-id="<?php echo $office?>"></i></td>
			<!--<td class="text-right"><?php echo @number_format(array_sum($total_base), 2)?></td>-->
			<!--<td class="text-right"><?php echo @number_format(array_sum($total_tax), 2)?></td>-->
			<td class="text-right"><?php echo @number_format(array_sum($total_dollars), 2)?></td>
			<td class="text-right"><?php echo @number_format(array_sum($total_process), 2)?></td>
